...
async def test_iframe_get(test_client, app):
    ...
    resp = await client.get('/v2/not_found')
    assert resp.status == 200
    resp_data = await resp.text()
    assert '<body style="width:px; height:px; background: transparent;"></body>' in resp_data


async def test_iframe_head(test_client, app):
    client = await test_client(app)
    resp = await client.head('/v2/not_found')
    assert resp.status == 200
...
async def test_iframe_post(test_client, app):
    client = await test_client(app)
    resp = await client.post('/v2/not_found')
    assert resp.status == 200


async def test_iframe_options(test_client, app):
    client = await test_client(app)
    resp = await client.options('/v2/not_found')
    assert resp.status == 200
    assert await resp.text() == ''
...
async def test_iframe_post_valid(test_client, app, faker):
    ...
    resp = await client.post('/v2/not_found', data=payload, headers=headers)
    assert resp.status == 200
